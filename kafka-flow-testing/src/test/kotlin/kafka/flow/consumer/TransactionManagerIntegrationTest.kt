...
class TransactionManagerIntegrationTest {
    ...
    fun committingTransaction_changesCommittedOffsets(): Unit = runTest {
        ...
        Await().untilAsserted {
            val committedOffsets: Map<TopicPartition, Long> = admin.listConsumerGroupOffsets("test-client").partitionsToOffsetAndMetadata().get().mapValues { (_, value) -> value.offset() }
