...
public class TransactionManager(private val maxOpenTransactions: Int) {
    ...
    public fun close(transaction: WithTransaction) {
        openedTransactions.compute(transaction) { _, value ->
            if (value == null) {
                logger.debug("[TransactionManager] Trying to close a transaction that doesn't exists $transaction")
            }
    ...
    public suspend fun rollbackAndCommit(client: KafkaFlowConsumerWithGroupId<*>) {
        ...
        client.commit(offsetsToCommit) { result ->
            if (result.isSuccess) {
                removeCommittedOffsets(result.getOrThrow())
            } else logger.error("[TransactionManager] Error while committing offsets ($offsetsToCommit), the system will retry", result.exceptionOrNull())
        }
    }
